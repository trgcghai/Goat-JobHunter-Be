...
class FriendRequestUnblockTest {
    ...
    @Test
    void handleUnblockUser_nullTargetId_throwsInvalid() {
        org.junit.jupiter.api.Assertions.assertThrows(iuh.fit.goat.exception.InvalidException.class, () ->
                friendRequestService.handleUnblockUser(null)
        );
    }

    @Test
    void handleUnblockUser_negativeTargetId_throwsInvalid() {
        org.junit.jupiter.api.Assertions.assertThrows(iuh.fit.goat.exception.InvalidException.class, () ->
                friendRequestService.handleUnblockUser(-1L)
        );
    }

    @Test
    void handleUnblockUser_selfUnblock_throwsInvalid() {
    ...
    @Test
    void handleUnblockUser_unauthenticated_throwsInvalid() {
        try (MockedStatic<SecurityUtil> mockedSecurity = mockStatic(SecurityUtil.class)) {
            mockedSecurity.when(SecurityUtil::getCurrentUserLogin).thenReturn(Optional.empty());

            org.junit.jupiter.api.Assertions.assertThrows(iuh.fit.goat.exception.InvalidException.class, () ->
                    friendRequestService.handleUnblockUser(2L)
            );
        }
    }

    @Test
    void handleUnblockUser_currentAccountMissing_throwsInvalid() {
        try (MockedStatic<SecurityUtil> mockedSecurity = mockStatic(SecurityUtil.class)) {
            mockedSecurity.when(SecurityUtil::getCurrentUserLogin).thenReturn(Optional.of("actor@example.com"));
    ...
    @Test
    void handleUnblockUser_currentAccountIsNotUser_throwsInvalid() {
    ...
    @Test
    void handleUnblockUser_targetNotFound_throwsNotFound() {
        try (MockedStatic<SecurityUtil> mockedSecurity = mockStatic(SecurityUtil.class)) {
            mockedSecurity.when(SecurityUtil::getCurrentUserLogin).thenReturn(Optional.of("actor@example.com"));
            when(accountRepository.findByEmailAndDeletedAtIsNull("actor@example.com")).thenReturn(Optional.of(actor));
    ...
    @Test
    void handleUnblockUser_success_deletedCountPositive_publishesEvent() throws Exception {
        ...
        try (MockedStatic<SecurityUtil> mockedSecurity = mockStatic(SecurityUtil.class)) {
            ...
            assertThat(captor.getValue().getType()).isEqualTo(FriendshipRealtimeEventType.USER_UNBLOCKED);
            assertThat(captor.getValue().getRelationshipState()).isEqualTo(RelationshipState.NONE);
            assertThat(captor.getValue().getActorPrincipal()).isEqualTo("actor@example.com");
            assertThat(captor.getValue().getTargetPrincipal()).isEqualTo("target@example.com");
        }
    }

    @Test
    void handleUnblockUser_success_deletedCountZero_doesNotPublishEvent() throws Exception {
        when(userRepository.findByAccountIdAndDeletedAtIsNull(2L)).thenReturn(Optional.of(target));
        when(userRelationshipRepository.lockPairForTransactionByCanonicalIds(1L, 2L)).thenReturn(new Object());
        when(userRelationshipRepository.hardDeleteByCanonicalPairAndState(1L, 2L, RelationshipState.BLOCKED)).thenReturn(0);
    ...
    @Test
    void handleUnblockUser_success_highToLowPair_usesCanonicalOrdering() throws Exception {
        ...
        when(userRelationshipRepository.hardDeleteByCanonicalPairAndState(3L, 10L, RelationshipState.BLOCKED)).thenReturn(1);
        ...
        try (MockedStatic<SecurityUtil> mockedSecurity = mockStatic(SecurityUtil.class)) {
            ...
            FriendRequestResponse response = friendRequestService.handleUnblockUser(3L);

            assertThat(response.getSenderId()).isEqualTo(10L);
            assertThat(response.getReceiverId()).isEqualTo(3L);
            assertThat(response.getRelationshipState()).isEqualTo(RelationshipState.NONE);

            verify(userRelationshipRepository).hardDeleteByCanonicalPairAndState(3L, 10L, RelationshipState.BLOCKED);
        }
    }

    @Test
    void handleUnblockUser_responseAlwaysUsesNoneRelationshipState() throws Exception {
        when(userRepository.findByAccountIdAndDeletedAtIsNull(2L)).thenReturn(Optional.of(target));
        when(userRelationshipRepository.lockPairForTransactionByCanonicalIds(1L, 2L)).thenReturn(new Object());
        when(userRelationshipRepository.hardDeleteByCanonicalPairAndState(1L, 2L, RelationshipState.BLOCKED)).thenReturn(1);
