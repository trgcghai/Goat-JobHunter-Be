...
public class CommentServiceImpl implements CommentService {
    ...
    public void handleDeleteComment(long id) {
        ...
        // Đếm số lượng comment sẽ bị xóa (bao gồm cả comment con)
        int deletedCount = countComments(comment);

        // Xóa đệ quy từ con lên cha
        deleteCommentRecursively(comment);

        // Cập nhật số lượng comment trong blog
    ...
    private void deleteCommentRecursively(Comment comment) {
        ...
        if (comment.getChildren() != null && !comment.getChildren().isEmpty()) {
            List<Comment> children = new ArrayList<>(comment.getChildren());
            for (Comment child : children) {
                deleteCommentRecursively(child);
            }
        }

        // Detach khỏi parent trước khi xóa
        if (comment.getParent() != null) {
            comment.getParent().getChildren().remove(comment);
        }

        // Xóa comment hiện tại và flush ngay
        this.commentRepository.delete(comment);
        this.commentRepository.flush();
    }

    private int countComments(Comment comment) {
        int count = 1;
        if (comment.getChildren() != null && !comment.getChildren().isEmpty()) {
